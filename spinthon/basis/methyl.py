...
class methyl_A_E(zeemanProductBasis):
    def __init__(self, spinSystem):
        ...
        self.KetsNew.extend(self.Kets[7:7+multiplicity])
        ...
        for k in range(multiplicity):
            self.KetsNew.append(1/np.sqrt(3)*
                                 (self.Kets[4*multiplicity + k] +
                                  ...
                                  np.conjugate(epsilon)*self.Kets[2*multiplicity + k]))
        ...
        for k in range(multiplicity):
            self.KetsNew.append(1/np.sqrt(3)*
                                 (self.Kets[6*multiplicity + k] +
                                  ...
                                  np.conjugate(epsilon)*self.Kets[3*multiplicity + k]))

        #Eb states are like Ea with conjugation swapped; -1/2
        for k in range(multiplicity):
            self.KetsNew.append(1/np.sqrt(3)*
                                 (self.Kets[4*multiplicity + k] +
                                  np.conjugate(epsilon)*self.Kets[1*multiplicity + k] +
        ...
        for k in range(multiplicity):
            self.KetsNew.append(1/np.sqrt(3)*
                                 (self.Kets[6*multiplicity + k] +
                                  np.conjugate(epsilon)*self.Kets[5*multiplicity + k] +
